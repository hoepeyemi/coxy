...
async function setupTwitterBot() {
  console.log('🤖 Twitter Bot Database Setup');
  console.log('================================\n');
  ...
  try {
    ...
    if (missingTables.length === 0) {
      ...
      console.log('You can start the Twitter bot with: npm start');
      return;
    }
    ...
    console.log('2. Copy the contents of: supabase-twitter-schema.sql');
    ...
    const sqlFilePath = path.join(process.cwd(), 'supabase-twitter-schema.sql');
    ...
    if (fs.existsSync(sqlFilePath)) {
      ...
      console.log('❌ supabase-twitter-schema.sql file not found');
      console.log('Please make sure the file exists in the twitter-bot directory');
    }
